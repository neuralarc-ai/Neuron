...
export async function generatePayslipPDF(data: PayslipData): Promise<void> {
  ...
  y += 25; // Added 8 units of top margin to PAYMENT ADVICE title
  ...
  const detailsHeight = data.employee.agreementRefId ? 23 : 16; // Increased to 25 to add proper bottom margin for Agreement Ref
